...
**Last updated:** 2026-07-23 (Session 21 — **MANUAL SNIPER MODE SHIPPED** (paste-a-URL → score → draft → pin, zero Apify — the GATE-2 data flow is back on) · **composer reach-lint** (Rank-2 rules as advisory hints in all 3 composers) · **unified daily-plan home card** (assignment + top topic + outcome/CSV reminders). ⚠ **PROD MIGRATION PENDING** — `source` column not yet applied (see Session 21 NEXT #1).)
...
**⚠ ONE BLOCKER LEFT — PROD MIGRATION NOT APPLIED.** The session's MCP `apply_migration` call was denied by the permission classifier. `supabase/migrations/20260722_sniper_manual_source.sql` is committed but NOT applied to PROD `vzxpakxjnuaesfxihyvl`. Until applied, the manual-sniper submit fails with a contained error message ("saving manual alert failed: …column source…") — everything else on the site is unaffected (no read path selects `source`). Apply via Supabase MCP/SQL editor:
```sql
...
**NEXT (priority order):**
1. **Owner: apply the `source` migration to PROD** (SQL above) → then paste one real tweet URL on `/engage` end-to-end (score → pin → Send opens composer → confirm → row `status='acted'`, `source='manual'`).
